...
df_real=pd.read_excel('../../GD-driven/A.DataGeneration/TestData/Data Set'+obj_fun+'/real_parts_eigenvalues_OPF_'+obj_fun+'.xlsx')
df_imag=pd.read_excel('../../GD-driven/A.DataGeneration/TestData/Data Set'+obj_fun+'/imag_parts_eigenvalues_OPF_'+obj_fun+'.xlsx')
...
res_opf=pd.read_excel('../A.DataGeneration/TestData/Data Set'+obj_fun+'/Test_data_OPF_'+obj_fun+'.xlsx')
comp_opf=pd.read_excel('../../GD-driven/A.DataGeneration/TestData/Data Set'+obj_fun+'/comp_burn_OPF_'+obj_fun+'.xlsx')
...
if obj_fun=='Min_P_SG':
    ...
    gs_lower=GridSpec(4,1)
    
    
    ax = fig.add_subplot(gs_lower[0])
    ax.scatter(res_opf[['PLTOT']]*100,DI_crit_eig_OPF,c='r',linewidth=3, label='OPF')
    ax.scatter(res_opf[['PLTOT']]*100,DI_SSCOPF,c='g',linewidth=3,label='SSC-OPF')
    ax.plot(res_opf[['PLTOT']]*100,DI_Pred_crit_eig_SSCOPF,c='y',linewidth=3, linestyle='dotted', label='Predicted SSC-OPF DI')
    
    ax.tick_params(axis='x', labelsize= 20)
    ax.tick_params(axis='y', labelsize= 20)
    ax.set_ylabel(r'$DI$',fontsize=20)#, labelpad=20)
    ax.set_xlabel(r'$P_{D}$ [MW]',fontsize=20)#,labelpad=20)#, loc='left')
    ax.grid()
    
    ax1 = fig.add_subplot(gs_lower[1])
    ax1.scatter(res_opf[['PLTOT']]*100,sgtot_opf,c='r',linewidth=4)#, linestyle='dotted')
    ax1.scatter(res_opf[['PLTOT']]*100,obj_fun_val_sscopf,c='g',linewidth=3)
    
    ax1.tick_params(axis='x', labelsize= 20)
    ax1.tick_params(axis='y', labelsize= 20)
    #ax1.set_ylabel(r'$P_{SG,NR}/P_{SG,NR}^{max}$',fontsize=20, labelpad=5)
    ax1.set_xlabel(r'$P_{D}$ [MW]',fontsize=20)#,labelpad=20)#, loc='left')
    ax1.set_ylim(-10,110)
    ax1.set_yticks([0,50,100])
    # ax1.set_yticklabels(['$P_{SG,NR}^{min}$ \n [MW]'])
    ax1.set_yticklabels(['$P_{SG,NR}^{min}$',' ','$P_{SG,NR}^{max}$'])
    
    
    ax1.grid()
    
    ax1_1=fig.add_subplot(gs_lower[2])
    ax1_1.scatter(res_opf[['PLTOT']]*100,res_sscopf[['Rdroop']],c='r',linewidth=3)
    ax1_1.scatter(res_opf[['PLTOT']]*100,res_opf[['Rdroop']],c='g',linewidth=3)
    ax1_1.set_yticks([0.05,0.1])
    ax1_1.set_yticklabels(['0.05','0.1'])
    ax1_1.tick_params(axis='x', labelsize= 20)
    ax1_1.tick_params(axis='y', labelsize= 20)
    ax1_1.set_xlabel(r'$P_{D} [MW]$',fontsize=20)#,labelpad=20)#, loc='left')
    ax1_1.set_ylabel(r'$R$',fontsize=20, labelpad=-10)
    ax1_1.grid()
    
    ax1_2=fig.add_subplot(gs_lower[3])
    ...
    # ax1_2.set_yticks([0.05,0.1])
    # ax1_2.set_yticklabels(['0.05','0.1'])
    ...
    ax.set_title(r'GD\&CT-driven Case - min\{$P_{SG,NR}$\} problem', fontsize=20)
    #fig.tight_layout()
    fig.legend(loc='lower center', ncol=3, fontsize=18)
    
    gs_lower.update(left=0.15,right=0.98,top=0.95,bottom=0.17,hspace=1)
    
 
elif obj_fun=='Min_P_losses':    

    fig = plt.figure(figsize=(8,11))
    gs_upper=GridSpec(5,1)
    
    ax2 = fig.add_subplot(gs_upper[0])
    ax3 = fig.add_subplot(gs_upper[1])
    ax4 = fig.add_subplot(gs_upper[2])
    ax1_1 = fig.add_subplot(gs_upper[3])
    ax1_2 = fig.add_subplot(gs_upper[4])
    ...
    ax2.set_title(r'GD\&CT-driven Case - min\{$P_{losses}$\} problem', fontsize=20)
    fig.legend(loc='lower center', ncol=3, fontsize=18)


    gs_upper.update(left=0.13,right=0.95, top=0.95,bottom=0.15,hspace=0.8)
