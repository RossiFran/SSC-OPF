...
elseif obj_fun == "Min_P_losses"
    ...
    fun = @(x)Min_P_losses(x,dat,branch);

end

options = optimoptions('fmincon','Display','off','MaxFunctionEvaluations',10000,'Algorithm','interior-point','MaxIterations',10000,'FunctionTolerance',1e-12);%,'PlotFcn','optimplotfvalconstr');
...
if obj_fun == "Min_P_SG"
    ...
    fval(jj) = Min_P_SG(sol);

elseif obj_fun == "Min_P_losses"
    
    fval(jj) = Min_P_losses(sol,dat,branch);
